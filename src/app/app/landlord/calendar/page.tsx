...
export default async function PartnerCalendarPage({
  ...
  const ctx = await requirePageLandlordOrg("landlord:manage_calendar");
    ...
    <div className="flex flex-col gap-6">
      ...
      {spaces.length > 1 && (
        <div className="flex flex-wrap gap-2">
          {spaces.map((space) => (
            <Link
              key={space.id}
              href={`/app/landlord/calendar?space=${space.id}&month=${month}`}
              className={clsx(
      ...
      <Card className="p-5">
        <MonthCalendar
          yearMonth={month}
          days={days}
          hrefForMonth={(m) => `/app/landlord/calendar?space=${selected.id}&month=${m}`}
        />
      </Card>
